    ...
    */
    ...
    class GoAnnotextGraph extends LitElement {
      constructor(...args) {
        ...
        this.height = 900;
        this.data = undefined;
        this.colorScale = Accent;
        this.simulation = void 0;
        this.link = void 0;
        this.node = void 0;

        this.ticked = () => {
          this.link.attr("x1", d => d.source.x).attr("y1", d => d.source.y).attr("x2", d => d.target.x).attr("y2", d => d.target.y).attr("stroke", this.colorScale[0]).attr("stroke-width", 1);
        ...
        this.initForceDisplay = () => {
          ...
          const svg$$1 = select(this.shadowRoot.getElementById("annotext-graph")); // link lines

          this.link = svg$$1.append("g").selectAll("line").data(this.data.edges).enter().append("line"); // node groups

          this.node = svg$$1.append("g").selectAll("circle").data(this.data.nodes).enter().append("g"); // node circles

          this.node.append("circle").attr("r", 5).attr("fill", this.colorScale[1]); // node text

          this.node.append("text").attr("dx", 12).attr("dy", ".35em").text(d => d.id);
          this.node.call(drag().on("start", this.dragstarted).on("drag", this.dragged).on("end", this.dragended));
      ...
      updated() {
        ...
        this.simulation = simulation(this.data.nodes).force("link", link().distance(100).links(this.data.edges).id(d => d.id)).force("charge", manyBody().distanceMin(10)).force("center", center(this.width / 2, this.width / 2)).on("tick", this.ticked);
        this.initForceDisplay();
      }
...
//# sourceMappingURL=go-annotext-graph.js.map
